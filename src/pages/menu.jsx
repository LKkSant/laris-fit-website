...
const [formPersonalizado, setFormPersonalizado] = useState({
    ...
    setItems((prev) =>
      prev.map((item) =>
        item.id === id
          ? {
              ...item,
              quantidade: Math.max(0, item.quantidade + valor),
            }
          : item
      )
    );
  ...
  const totalMarmitas = items.reduce(
...
`;
    ...
    const url = `https://wa.me/5514974008094"text=${encodeURIComponent(
      mensagem
    )}`;
  ...
  <div className="bg-white p-8 rounded-3xl shadow-lg mb-12">
    ...
    <div className="grid gap-4">
      ...
      <button
        onClick={() => {
          const mensagem = `
Olá, gostaria de solicitar um Cardápio Personalizado.

Nome: ${formPersonalizado.nome}

Objetivo:
${formPersonalizado.objetivo}

Restrições:
${formPersonalizado.restricoes}

Observações:
${formPersonalizado.observacoes}
`;
...
if (
  ...
  return;
          ...
          window.open(
            `https://wa.me/5514974008094?text=${encodeURIComponent(
              mensagem
            )}`,
            "_blank"
          );
